...
function getGuestDirectoryForEvent(eventId) {
    if (typeof window.getLocalGuestDirectoryForEvent === "function") {
        return window.getLocalGuestDirectoryForEvent(eventId);
    }

    const localSeeds = window.LocalGuestSeeds || {};
    return localSeeds[eventId] || {};
}

function mergeGuestDirectories(fallbackDirectory, remoteDirectory) {
    return {
        ...(fallbackDirectory || {}),
        ...(remoteDirectory || {})
    };
...
function buildRows(confirmations, guestDirectory) {
    ...
    configuredIds.forEach((id) => {
        ...
        if (guest && guest.activo === false) return;
